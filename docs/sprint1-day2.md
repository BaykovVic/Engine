...
Структуры / классы / enum:
- скрытый класс-реализация ILogger.
...
Структуры / классы / enum:
- скрытый класс-реализация IConfigService.
...
Структуры / классы / enum:
- скрытый класс-реализация VulkanRenderer.
...
Логика функций / методов:
- минимальный вершинный шейдер (позже дорастёт до PBR).
...
Логика функций / методов:
- минимальный фрагментный шейдер (позже дорастёт до PBR).
...
Функции / методы:
- привязка инструмента к панели.

Логика функций / методов:
- каждый класс-инструмент привязан к своей панели.
...
Логика функций / методов:
- пустая панель-заглушка на месте будущих панелей.
...
Логика функций / методов:
- тело за 1 с падает ≈4.9 м; куб замирает на полу; тело удерживается на heightfield; привязанный объект синхронно опускается.
...
Логика функций / методов:
- CI на Ubuntu: установка зависимостей, cmake --build, xvfb-run ctest; красный статус блокирует слияние.
...
Логика функций / методов:
- система обновляет только сущности с нужным компонентом; результат читается через storeFor<T>().get(...).
...
Логика функций / методов:
- entitiesWith возвращает только сущности с указанным набором компонентов.
