...
- **Packages** **[оба]** — ✅ список обнаруженных пакетов (имя/версия/статус) на
  ...
  ABI `package_install`.
  ✅ P4: managed-код в пакетах — `Runtime/*.cs` активных пакетов компилируется
  в общую пользовательскую сборку тем же csproj-пайплайном, классы попадают в
  пикер скриптов и работают в Play; активация/деактивация перекомпилирует
  (сброс кэшей классов в мосте), последний исчезнувший исходник выгружает
  сборку (collectible ALC). Дальше (P3+): вкладка Browse со статическим
  индексом реестра, учёт чексумм-дрейфа в UI, Update-flow.
