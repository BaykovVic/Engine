...
- **B18. Ввод** — делать непосредственно ПЕРЕД B3 (игровой UI без мыши
  невозможен). Сейчас есть только уровневый `Input.GetKey` с минимальным
  `KeyCode` (буквы/цифры/пробел/Esc/Enter/Tab/левые модификаторы/стрелки):
  - [ ] Краевые события `GetKeyDown`/`GetKeyUp` (per-frame защёлки; для
        `OnFixedUpdate` определить семантику сразу — у Unity это классическая
        ловушка потерянных нажатий).
  - [ ] Мышь: позиция/кнопки/колесо. Платформенный слой уже эмитит всё
        (X11: MouseMove/Button/Wheel в `x11_window_system.cpp`), но плеер
        события роняет, в reverse-API и managed их нет — достроить трубу.
  - [ ] Полнота KeyCode (F-клавиши, правые модификаторы, Backspace/Delete…).
  - [ ] Оси/action maps — после мыши, по потребности контента.
