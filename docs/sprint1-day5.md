...
- повороты и композиция трансформов дают ожидаемые значения (rotate/compose/invCompose).
...
Логика функций / методов:
- иерархия и мировые трансформы корректны; поле каждого из 5 типов пишется и читается без потерь.
